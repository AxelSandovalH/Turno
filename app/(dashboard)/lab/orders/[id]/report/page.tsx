...
export default async function ReportPage({ params }: Props) {
    ...
    .select(`
      ...
      tests:lab_order_tests(
        ...
        results:lab_order_results(
          ...
          captured:staff!lab_order_results_captured_by_fkey(name)
        )
      )
    `)
    .eq('id', id)
  ...
  const customer = order.customer as unknown as {
    name: string | null; phone: string; date_of_birth: string | null; gender: string | null
  } | null
  const tests = order.tests as unknown as {
    id: string
    test: { name: string } | null
    results: { value: string | null; unit: string | null; ref_range: string | null; captured_at: string | null; analyte: { name: string } | null; captured: { name: string } | null }[]
  }[]

  const withResults = tests.filter(t => t.results.length > 0)
  if (withResults.length === 0) redirect(`/lab/orders/${id}`)
  ...
  const capturedBy = withResults.flatMap(t => t.results).find(r => r.captured?.name)?.captured?.name
  ...
  return (
    <DocShell
      ...
      docTitle="Resultados"
      folio={order.folio}
      dateLabel={format(new Date(order.created_at), "d 'de' MMMM 'de' yyyy", { locale: es })}
      previewLabel={`Reporte de resultados ${order.folio}`}
    >
      {/* Paciente */}
      ...
      <div className="flex justify-end pt-10">
        <div className="text-center w-64">
          <div className="border-t border-zinc-400 pt-2">
            <p className="text-sm font-semibold">{capturedBy ?? ''}</p>
      ...
      <p className="text-center text-[10px] text-zinc-400 mt-10">
        Este reporte es informativo y debe ser interpretado por un médico. {organization.name}
      </p>
    </DocShell>
  )
}
